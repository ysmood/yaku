...
module.exports = function (task, option) {
    ...
    task('lint', 'lint js files', function () {
        return kit.spawn('eslint', ['*.js', 'src/*.js', 'test/*.js']);
    });
    ...
    task('test-browser', ['browser'], 'use puppeteer to test', function () {
        return require('./test/puppeteer');
    });
    ...
    task('browser', 'run test on browser', function () {
        return kit.spawn('webpack');
    });
};
